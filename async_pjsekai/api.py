...
class API:
    ...
    async def get_asset_bundle_info(
        ...
        async with self.session.get(url,headers=self._generate_headers(system_info)) as response:
            try:
                response.raise_for_status()
                ...
                if response.status == 426:
                    ...
                    raise
            return self._unpack(await response.read(), enable_asset_bundle_info_encryption)
        ...
        try:
            try:
                ...
                if response.status == 426:
                    ...
                    raise
            yield AssetBundle(obfuscated_chunks=response.content.iter_chunked(chunk_size))
        finally:
            response.close()
    ...
    async def authenticate(self, user_id: Union[int, str], credential: str) -> dict:
        responseDict: dict = await self.request("PUT", f"user/{user_id}/auth", data = { "credential": credential })
    ...
    async def get_master_data(self, data_version: Optional[str] = None) -> dict:
        if data_version is None:
            ...
            return await self.request("GET", f"suite/master", system_info=self.system_info.copy(update={"data_version":data_version}))

    async def get_notices(self) -> dict:
        return await self.request("GET", f"information")

    async def get_user_data(self, user_id: Union[int, str], name: Optional[str] = None) -> dict:
        params = {
    ...
    async def get_login_bonus(self, user_id: Union[int, str]) -> dict:
        return await self.request("PUT", f"user/{user_id}/home/refresh", data = {
            "refreshableTypes":[
    ...
    async def set_tutorial_status(self, user_id: Union[int, str], tutorial_status: TutorialStatus) -> dict:
        return await self.request("PATCH", f"user/{user_id}/tutorial", data = { "tutorialStatus": tutorial_status.value })

    async def generate_transfer_code(self, user_id: Union[int, str], password: str) -> dict:
        return await self.request("PUT", f"user/{user_id}/inherit", data = { "password": password })
    ...
    async def checkTransferCode(self, transfer_code: str, password: str) -> dict:
        ...
        return await self.request("POST", f"inherit/user/{transfer_code}", params=params, headers=header)
    ...
    async def gacha(self, user_id: Union[int, str], gacha_id: int, gacha_behavior_id: int) -> dict:
        return await self.request("PUT", f"user/{user_id}/gacha/{gacha_id}/gachaBehaviorId/{gacha_behavior_id}")

    async def receive_presents(self, user_id: Union[int, str], present_ids: List[str]) -> dict:
        return await self.request("POST", f"user/{user_id}/present", data = {
    ...
    async def start_solo_live(
        ...
        return await self.request("POST", f"user/{user_id}/live", data = {
            "musicId": music_id,
    ...
    async def end_solo_live(
        ...
        return await self.request("PUT", f"user/{user_id}/live/{live_id}", data = {
            "score": score,
    ...
    async def get_event_rankings(
        ...
        return await self.request("GET", f"user/{user_id}/event/{event_id}/ranking", params=params)
    ...
    async def get_rank_match_rankings(
        ...
        return await self.request("GET", f"user/{user_id}/rank-match-season/{rank_match_season_id}/ranking", params=params)

    async def get_room_invitations(self, user_id: Union[int, str]) -> dict:
        return  await self.request("GET", f"user/{user_id}/invitation")
    
    async def send_friend_request(self, user_id: Union[int, str], target_user_id: Union[int, str], message: Optional[str] = None) -> dict:
        return await self.request("POST", f"user/{user_id}/friend/{target_user_id}", data={
            "message": message, 
            "friendRequestSentLocation": "id_search",
        })

    async def reject_friend_request(self, user_id: Union[int, str], request_user_id: Union[int, str]) -> dict:
        params = {
            "type": "reject_friend_request",
        }
        return await self.request("DELETE", f"user/{user_id}/friend/{request_user_id}", params=params)

    async def accept_friend_request(self, user_id: Union[int, str], request_user_id: Union[int, str]) -> dict:
        return await self.request("PUT", f"user/{user_id}/friend/{request_user_id}")

    async def remove_friend(self, user_id: Union[int, str], friend_user_id: Union[int, str]) -> dict:
        params = {
            "type": "release_friend",
        }
        return await self.request("DELETE", f"user/{user_id}/friend/{friend_user_id}", params=params)
