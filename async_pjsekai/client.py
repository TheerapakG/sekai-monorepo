...
class Client:
    ...
    def _auto_session_refresh(func: Callable[Concatenate["Client",P], Coroutine[None, None, R]]) -> Callable[Concatenate["Client",P], Coroutine[None, None, R]]: # type: ignore[misc]
        @wraps(func)
        async def wrapper_auto_session_refresh(self: "Client", *args: P.args, **kwargs: P.kwargs) -> R:
            try:
    ...
    def _auto_update(func: Callable[Concatenate["Client",P], Coroutine[None, None, R]]) -> Callable[Concatenate["Client",P], Coroutine[None, None, R]]: # type: ignore[misc]
        async def wrapper_auto_update(self: "Client", *args: P.args, **kwargs: P.kwargs) -> R:
            if self.auto_update:
                try:
                    try:
                        return await func(self, *args, **kwargs)
    ...
    async def update_app(self, app_version: Optional[str] = None, app_hash: Optional[str] = None, multi_play_version: Optional[str] = None) -> None:
    ...
    async def refresh_signed_cookie(self) -> AbstractCookieJar:
        cookies: Dict[str, str] = {k:v for k,v in (cookie.split("=") for cookie in (c.strip() for c in (await self.api_manager.get_signed_cookie()).split(";")) if cookie!="")}
    ...
    async def get_notices(self) -> List[Information]:
        return [Information(**information) for information in (await self.api_manager.get_notices())["informations"]]
    ...
    async def transfer_check(self, transfer_code: str, password: str) -> dict:
        response: dict = await self.api_manager.checkTransferCode(transfer_code, password)
        return response

    @_auto_update
    @_auto_session_refresh
    async def transfer_in(self, transfer_code: str, password: str) -> dict:
        response: dict = await self.api_manager.generate_credential(transfer_code, password)
    ...
    @_auto_session_refresh
    @_auth_required
    async def send_friend_request(self, user_id: Union[int, str], message: Optional[str] = None) -> None:
