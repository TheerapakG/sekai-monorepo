...
from async_pjsekai.exceptions import TutorialEnded
from async_pjsekai.enums.enums import Unit
...
class TutorialStatus(Enum):
    ...
    def next(self, unit: Unit = Unit.LN) -> "TutorialStatus":
        if self is TutorialStatus.START:
            ...
            self is TutorialStatus.LN_OPENING
            or self is TutorialStatus.MMJ_OPENING
            or self is TutorialStatus.VBS_OPENING
            or self is TutorialStatus.WXS_OPENING
            or self is TutorialStatus.NIIGO_OPENING
        ):
